...
using namespace std;
struct Grade {
    int math = -1, phy = -1, chem = -1, cs = -1, nep = -1, eng = -1, sum = 0;
    float gpa = 0.0;
};
...
file << "GRADES\n";
for (const auto& student : students) {
    file << student.id << "," << student.grade.math << "," << student.grade.phy << ","
         << student.grade.chem << "," << student.grade.cs << "," << student.grade.nep << ","
         << student.grade.eng << "\n";
}
file << "END_GRADES\n";
    }
...
while (getline(file, line)) {
    ...
    if (tokens.size() == 7) {
        ...
        for (auto& student : students) {
            if (student.id == id) {
                student.grade.math = stoi(tokens[1]);
                student.grade.phy = stoi(tokens[2]);
                student.grade.chem = stoi(tokens[3]);
                student.grade.cs = stoi(tokens[4]);
                student.grade.nep = stoi(tokens[5]);
                student.grade.eng = stoi(tokens[6]);
                student.grade.sum = student.grade.math + student.grade.phy + student.grade.chem +
                                    student.grade.cs + student.grade.nep + student.grade.eng;
                student.grade.gpa = student.grade.sum / 6.0f;
                break;
            }
        }
    }
    ...
    for (auto& student : students) {
        if (student.id == sid) {
            cout << "Entering grades for " << student.name << ":\n";
            cout << "Math: "; cin >> student.grade.math;
            cout << "Physics: "; cin >> student.grade.phy;
            cout << "Chemistry: "; cin >> student.grade.chem;
            cout << "Computer Science: "; cin >> student.grade.cs;
            cout << "Nepali: "; cin >> student.grade.nep;
            cout << "English: "; cin >> student.grade.eng;
            cin.ignore();
...
void viewGrades() {
    ...
    cout << left << setw(10) << "ID" << setw(20) << "Name"
         << "Math Phy Chem CS Nep Eng | GPA\n";
    cout << string(70, '-') << "\n";

    for (const auto& student : students) {
        if (student.grade.math == -1) {
            cout << left << setw(10) << student.id << setw(20) << student.name
                 << "No grades entered.\n";
        } else {
            cout << left << setw(10) << student.id << setw(20) << student.name
                 << setw(5) << student.grade.math
                 << setw(5) << student.grade.phy
                 << setw(5) << student.grade.chem
                 << setw(5) << student.grade.cs
                 << setw(5) << student.grade.nep
                 << setw(5) << student.grade.eng
                 << "| " << fixed << setprecision(2) << student.grade.gpa << "\n";
        }
    }
}
